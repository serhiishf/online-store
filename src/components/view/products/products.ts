...
export function drawProducts(data: Product[] | []): void {
    ...
    if (products.length > 0) {
        products.forEach((item: Product) => {
            ...
            (<HTMLElement>(<HTMLElement>prodClone.querySelector('.card__icon-cart')).firstElementChild).setAttribute(
                'href',
                './assets/icons/sprite.svg#to-cart'
            );
